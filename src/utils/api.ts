...
const getToken = (): string | null => {
  ...
  console.log("token envoyé:", token ? token.slice(0, 20) + "..." : "NULL"); // ← ajoute
  return token;
};
...
export const api = {
  ...
  register: (data: {
    ...
    request<{ token: string; user: AuthUser }>("POST", "/auth/register", {
      ...data,
      role: "client",   // enforced on frontend too, backend ignores it anyway
    }),
